...
| **Cliente fora do horário** | **[DECIDIDO 2026-08-04]** Só **pedido fechado** fora de 8h–19h vira notificação no Telegram marcada como **"🌙 fora do horário"** — a marca sai na notificação de pedido novo (`/novo-pedido`, campo `foraHorario` no corpo) e no aviso do tópico IA (`/ia-evento` tipo `pedido`, mesmo campo). **Mensagem recebida fora do horário não avisa mais nada** (o evento `fora-horario` por mensagem foi removido do serviço): quem escreve às 22h escreve várias vezes, cada uma apitava, e alerta que vira ruído deixa de ser lido justamente no dia em que importa. O que **não** muda é a conversa: `checar_data` continua dizendo com todas as letras que a D'Luh não atende no horário pedido (`periodoPedido`/`janelaAtendivel` do `datas.js`) — isso é resposta ao cliente, não alerta pra dona. |
