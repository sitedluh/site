...
async function sbStatusConsultar(tel){
  ...
  try{
    ...
    _sbUltimosPedidos=pedidosAtivos; // disponibiliza os pedidos pro sbEditarPedido sem re-fetch
...
function sbNovoPedido(){
  sbEsconderInput();
  sbAddMsg('bot','Que delícia! 🎂 Boa notícia: você já está no lugar certo! É só fechar este chat, rolar a página e montar seu pedido aqui mesmo. 😉');
  sbNovoPedidoAjuda();
}
...
function sbNormalizaProdutoBot(row){
  if(row.nome!==undefined){
    return{nome:row.nome||'',tipo:row.tipo||'Outros',valorUnit:Number(row.valor)||0,qtdMin:Number(row.qtdMin)||1};
  }
  return{
...
async function sbProdutosCache(){
  ...
  return(d.produtos||d.items||[]).map(sbNormalizaProdutoBot).filter(p=>p.nome);
}
