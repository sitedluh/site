...
- **Nunca comparar telefone/waid inteiro e exato.** O WhatsApp entrega o número ora com o nono dígito, ora sem ele — comparação exata erra **silenciosamente** (achou o bug real da whitelist da IA em produção). Sempre comparar pelos **últimos 8 dígitos**, convenção já usada em `/status-pedido`, `/cancelar-pedido` e `iaNaWhitelist()`.